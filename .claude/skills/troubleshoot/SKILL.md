...
reach cluster DNS, its own namespace, and the public internet. Everything else is dropped
silently, which presents as a hang rather than a refusal. The Kubernetes API, other apps and
the 192.168.2.0/24 LAN are blocked deliberately.
...
- `Pending` forever with a PVC: this app should not have one — it is stateless.
